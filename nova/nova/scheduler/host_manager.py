...
class HostState(object):
    ...
    def update_from_compute_node(self, compute):
        ...
        LOG.info(_(" Befor addinf bandwidth "));

        if compute.get('total_bandwidth') != None:
            self.total_bandwidth = compute['total_bandwidth'];
        else:
            self.total_bandwidth = 1000;
        if compute.get('bandwidth_used') != None:
            self.used_bandwidth = compute['bandwidth_used'];
        else:
            self.used_bandwidth = 0;
